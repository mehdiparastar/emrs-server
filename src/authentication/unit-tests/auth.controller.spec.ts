...
describe('AuthController', () => {
  ...
  it('check create', async () => {
    expect(
      await authController.create({ name: 'test', email: 'test@test.com', password: 'test' }),
  ...
  it('check user profile', async () => {
    expect(
      authController.getProfile({ user: fakeUsersRepo[0] } as unknown as Request),
    ).toHaveProperty('id');
  });
  ...
  it('check approve user role', async () => {
    expect(
      await authController.approveUserRoles('1', { [UserRoles.admin]: true }),
    ).toHaveProperty('id');
  });
