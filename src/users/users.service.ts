...
export class UsersService {
  ...
  async changeUserRoles(
    ...
    const isSuperUser = user.roles.includes(UserRoles.superUser)

    const updateUserRoles = await this.update(id, {
      roles: isSuperUser ? [UserRoles.superUser, ...getRolesArray(newRoles)] : getRolesArray(newRoles),
    });
